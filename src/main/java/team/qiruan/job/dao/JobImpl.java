...
public class JobImpl implements JobDao{
    ...
    public Job_more search(int id) {
        List<Job_more> list = jdbcTemplate.query("SELECT job.id,userinfo.avatar,userinfo.name,userinfo.sex, job.address,job.salary,job.time,job.phone,job.qq,job.mail,job.detail FROM job,userinfo WHERE job.uid=userinfo.uid and job.id = ?", new Object[]{id}, new BeanPropertyRowMapper<Job_more>(Job_more.class));
    ...
    public List<Job_less> getAllJobs_Less() {
        List<Job_less> res= jdbcTemplate.query("SELECT job.id,userinfo.avatar,userinfo.name,userinfo.sex, job.address,job.salary,job.time  FROM job,userinfo WHERE userinfo.uid=job.uid", new Object[]{}, new BeanPropertyRowMapper<Job_less>(Job_less.class));
        if(res.isEmpty()){
            return null;
        }
        return res;
